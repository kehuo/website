    ...
    request.onreadystatechange = function () {
        if (request.readyState == 4) {
            if ((request.status >= 200 && request.status < 300) || request.status == 304) {
                ...
                document.querySelector('#mnist-pad-result').innerHTML = data.number;
                document.querySelector('#mnist-pad-prob').innerHTML = data.prob;
            }
            ;
        }
    };
...
clearBtn.addEventListener("click", function (event) {
    mnistPad.clear();
    document.querySelector('#mnist-pad-result').innerHTML = ''
    document.querySelector('#mnist-pad-prob').innerHTML = ''
});
